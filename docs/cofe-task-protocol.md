...
- `APPLY_PATCH` is a list of entries. Each entry starts with `file: <path>` and optionally adds `dry_run: true|false` (canonical) or `dryRun: true|false` (compatibility alias supported by the runner), plus `force: true|false`, before the next `file:`.
