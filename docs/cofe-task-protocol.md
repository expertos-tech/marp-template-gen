...
- `APPLY_PATCH` is a list of entries. Each entry starts with `file: <path>` and optionally adds `dry_run: true|false` (use in new files) or `dryRun: true|false` (deprecated alias supported by `scripts/run-task.mjs` for backward compatibility), plus `force: true|false`, before the next `file:`.
